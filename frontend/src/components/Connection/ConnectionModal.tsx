...
interface ConnectionModalProps {
  open: boolean;
  onClose: () => void;
  connection?: Connection | null;  // 编辑时传入
  mode: 'add' | 'edit';
}

const ConnectionModal: React.FC<ConnectionModalProps> = ({ open, onClose, connection, mode }) => {
    ...
    if (open) {
      ...
      if (mode === 'edit' && connection) {
        form.setFieldsValue({
          ...
          rememberCredential: connection.rememberCredential || false,
        });
      }
    }
  }, [open, form, mode, connection]);
    ...
    try {
      if (mode === 'edit' && connection) {
        ...
        const updateData: any = {
          ...
          username: values.username,
          authType: values.authType,
          rememberCredential: values.rememberCredential,
        };
        ...
        if (values.privateKey) {
          updateData.privateKey = values.privateKey;
        }
        
        const res = await connectionApi.update(connection.id, updateData);

        if (res.data.code === 0) {
          message.success('连接更新成功');
          updateConnection(connection.id, res.data.data);
          onClose();
        } else {
      ...
      message.error(mode === 'edit' ? '更新连接失败' : '创建连接失败');
    }
  };
  ...
  return (
    <Modal
      title={isEdit ? '编辑 SSH 连接' : '新增 SSH 连接'}
      open={open}
      onCancel={onClose}
      footer={null}
      width={500}
    >
      ...
      >
        ...
        {authType === 'PASSWORD' ? (
          <Form.Item
            name="password"
            label={isEdit ? '新密码（留空保持不变）' : '密码'}
            rules={isEdit ? [] : [{ required: true, message: '请输入密码' }]}
          >
            <Input.Password placeholder={isEdit ? '输入新密码或留空' : 'SSH 登录密码'} />
          </Form.Item>
        ) : (
          <Form.Item
            name="privateKey"
            label={isEdit ? '新私钥（留空保持不变）' : '私钥'}
            rules={isEdit ? [] : [{ required: true, message: '请输入私钥' }]}
          >
            <Input.TextArea
              rows={4}
              placeholder={isEdit ? '粘贴新私钥或留空' : '粘贴 SSH 私钥内容'}
            />
          </Form.Item>
        )}
